...
class MPCLogger:
    ...
    def plot_results(self, nq: int) -> str:
        ...
        ax0_twin.plot(timestamps, ee_errors, 'k:', linewidth=2, alpha=0.5, label='Error')
        ...
        for i in range(nq):
            axes[1].plot(timestamps, joint_states[:, i], label=f'q{i+1}', linewidth=1.5)
        ...
        ax2_twin.axhline(y=10, color='orange', linestyle='--', alpha=0.7, 
                        linewidth=2, label='Real-time limit (10ms)')
