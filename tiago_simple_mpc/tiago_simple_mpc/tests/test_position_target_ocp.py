...
from tiago_simple_mpc.mpc.build_position_target_ocp import PositionOCPConfig, build_position_target_ocp
...
class OCPReachingNode(Node):
    ...
    def __init__(self):
        ...
        # Solution storage
        self.xs_solution = None
        self.us_solution = None
        ...
        self.target_position = self.ocp_config.default_target_position
    ...
    def sensor_callback_oneshot(self, msg):
        ...
        try:
            ...
            ee_measured = self.data.oMf[self.frame_id].translation.copy()
            ...
            self.viz.viewer["target_sphere"].set_transform(
                tf.translation_matrix(self.target_position)
            )
            ...
            if converged:
                # Display final state
                nq = self.model.nq
                q_final = self.mpcocp.solver.xs[-1][:nq]
    ...
    def solve_ocp(self):
        ...
        xs_init = [self.x_measured] * (self.ocp_config.horizon_length + 1)
        us_init = [self.us_0] * self.ocp_config.horizon_length
        ...
        # Store solution
        self.xs_solution = self.mpcocp.solver.xs
        self.us_solution = self.mpcocp.solver.us
